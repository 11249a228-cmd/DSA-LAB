...
char peek(Stack *s) { if(!isEmpty(s)) return s->arr[s->top]; return 0; }
int precedence(char op) { switch(op) { case '+': case '-': return 1; case '*': case '/': return 2; case '^': return 3; } return 0; }
...
void infixToPostfix(char infix[], char postfix[]) {
    ...
    for(i=0; infix[i]!='\0'; i++) {
        ...
        if(isalnum(c)) postfix[k++] = c;
            ...
            while(!isEmpty(&s) && precedence(peek(&s)) >= precedence(c)) postfix[k++] = pop(&s);
            push(&s, c);
        }
    }
